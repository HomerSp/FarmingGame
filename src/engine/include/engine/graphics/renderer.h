...
public:
    ...
    virtual void drawText(const Types::Rect<>& dst, const std::string& text, const graphics::Color& color, int32_t size = -1, Types::TextAlign align = Types::TextAlign(), std::string type = "", bool shadow = false) = 0;
    virtual void drawOverlay(const Types::Point<>& dst, Overlay& overlay, uint32_t lightsCount, float mod) = 0;
    virtual void drawParticles(const engine::Types::Point<>& dst, const engine::Particles& particles, bool round) = 0;
    ...
    virtual void drawTexturesAnim(const engine::Types::Point<>& dst, engine::graphics::Texture* texture, engine::graphics::Buffer* buffer, float_t animFrame, uint32_t count = 1) = 0;
